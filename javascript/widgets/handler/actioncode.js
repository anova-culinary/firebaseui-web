...
firebaseui.auth.widget.handler.handlePasswordReset = function(
  ...
  app.registerPending(
      app.getAuth()
          ...
          .then(
              function(email) {
                AnalyticsClient.trackWithPlatform("PasswordResetLinkOpened", {
                  email: email
                });
                ...
                var component = new firebaseui.auth.ui.page.PasswordReset(
                    email, function() {
                      firebaseui.auth.widget.handler.resetPassword_(
                          ...
                          opt_onContinueClick);
                      AnalyticsClient.trackWithPlatform("PasswordResetPasswordSubmitted", {
                          email: email
                      });
                    });
...
firebaseui.auth.widget.handler.resetPassword_ = function(
  ...
  app.registerPending(component.executePromiseRequest(
      ...
      function(resp) {
        ...
        AnalyticsClient.trackWithPlatform("PasswordResetSucceeded", {
            email: email
        });
      },
      function(error) {
